...
function ZoneDivider({ zone }: { zone: NonNullable<TableRow['zone']> }) {
  return (
    <div className="flex items-center gap-2 px-3.5 pt-2 pb-1.5">
      <span className="h-[3px] w-3.5 rounded-[2px]" style={{ background: zone.color }} />
      <span className="label-caps text-[9px] text-ink-secondary">{zone.name}</span>
  ...
  return (
    <div>
      ...
      <div className="mb-2.5 flex flex-wrap gap-1.5">
        {TABLE_LEAGUES.map((k) => {
          ...
          return (
            <button
              key={k}
              onClick={() => pickLeague(k)}
              aria-pressed={on}
              className={[
                'cursor-pointer rounded-full border-[1.5px] px-2.5 py-1 text-[11.5px] font-semibold whitespace-nowrap',
                on ? 'border-transparent text-white' : 'border-line-strong text-ink-secondary opacity-55',
              ].join(' ')}
      ...
      {legendOpen && (
        <div className="mb-3 rounded-lg border border-line bg-surface p-3">
            ...
            <div key={z.name} className="flex items-start gap-2 border-b border-line py-2 last:border-b-0">
              ...
              <div className="min-w-0 flex-1">
                <div className="text-[12px] font-semibold">
                  {z.name}{' '}
                  <span className="font-mono text-[9.5px] font-medium text-ink-muted">
          ...
          <div className="mt-1 text-[10.5px] leading-relaxed text-ink-secondary">
            ...
            to newest, W / D / L lettered, not colour alone.
          </div>
      ...
      <div className="-mx-5 md:hidden">
        <div className="sticky top-0 z-10 grid h-7 grid-cols-[3px_30px_34px_1fr_38px_38px] items-center border-y border-t-line border-b-line-strong bg-surface-alt pr-3.5">
          <div />
        ...
        {rows.map((r) => {
          const zoneStart = showZones && r.zone && rows.find((x) => x.zone?.name === r.zone!.name) === r
          const open = openRow === r.teamId
          return (
            <div key={r.teamId}>
              {zoneStart && r.zone && <ZoneDivider zone={r.zone} />}
              <button
                onClick={() => setOpenRow(open ? null : r.teamId)}
                aria-expanded={open}
                aria-label={`${r.rank}. ${r.name}, ${r.pts} points from ${r.played} played`}
                className="grid min-h-[54px] w-full cursor-pointer grid-cols-[3px_30px_34px_1fr_38px_38px] items-center border-b border-line bg-surface pr-3.5 text-left"
              >
                <div className="self-stretch" style={{ background: showZones ? (r.zone?.color ?? 'transparent') : 'transparent' }} />
              ...
              {open && (
                <div className="border-b border-line-strong bg-surface-alt px-3.5 pt-3 pb-3.5">
                  ...
                  <div className="mt-0.5 text-[10.5px] text-ink-secondary">
                    {r.zone
                      ? `${r.zone.name} place (${r.zone.from === r.zone.to ? r.zone.from : `${r.zone.from}–${r.zone.to}`}). ${r.zone.note}`
                  ...
                  {r.next && (
                    <div className="mt-2.5 rounded-[5px] border border-line border-l-3 border-l-pitch bg-surface px-2.5 py-2">
                      <div className="label-caps text-[8.5px] text-ink-muted">Next fixture</div>
      ...
      <div className="hidden md:block">
        ...
        <div className="grid h-[34px] grid-cols-[3px_42px_32px_minmax(150px,1fr)_34px_28px_28px_28px_36px_36px_42px_46px_48px_100px_120px] items-center border-b border-line-strong">
          <div />
        ...
        {sorted.map((r) => {
          const zoneStart =
            showZones && r.zone && sorted.find((x) => x.zone?.name === r.zone!.name) === r
          return (
            <div key={r.teamId}>
              {zoneStart && r.zone && (
                <div className="flex items-center gap-2 pt-2.5 pb-1.5">
                  <span className="h-[3px] w-4 rounded-[2px]" style={{ background: r.zone.color }} />
                  <span className="label-caps text-[9.5px] text-ink-secondary">{r.zone.name}</span>
                  <span className="h-px flex-1 opacity-30" style={{ background: r.zone.color }} />
                </div>
              )}
              <div className="grid min-h-[46px] grid-cols-[3px_42px_32px_minmax(150px,1fr)_34px_28px_28px_28px_36px_36px_42px_46px_48px_100px_120px] items-center border-b border-line bg-surface hover:bg-surface-alt">
                <div className="self-stretch" style={{ background: showZones ? (r.zone?.color ?? 'transparent') : 'transparent' }} />
